...
async function getCommuneForEdit(id: string) {
...
export default async function EditCommunePage({
  ...
  if (!user) redirect(`/auth/sign-in?redirectTo=/communes/${id}/edit`)

  const commune = await getCommuneForEdit(id)
  if (!commune) notFound()

  if (commune.created_by !== user.id) redirect(`/communes/${id}`)
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className="mx-auto max-w-2xl px-6 py-12">
        ...
        <div className="rounded-2xl border border-zinc-200 bg-white p-8 dark:border-zinc-800 dark:bg-zinc-900">
          ...
          <EditCommuneForm commune={commune} />
        </div>
      </main>
    </div>
  )
